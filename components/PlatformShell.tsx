...
const nav = [
  ...
  { href: "/history", label: "Historik", icon: "o" },
];
const apps = [
  ...
  return (
    <div style={{display:'flex', minHeight:'100vh', background:'#070708', color:'#EDEDE9'}}>
      <aside style={{width:'280px', borderRight:'1px solid #1A1A1E', background:'#0E0E10', padding:'24px', display:'flex', flexDirection:'column', justifyContent:'space-between', position:'sticky', top:0, height:'100vh'}}>
        <div>
          <div style={{fontWeight:900, fontSize:'22px'}}>FRED <span style={{background:'#BFFF00', color:'black', padding:'2px 8px', borderRadius:'999px', fontSize:'11px'}}>PRO</span></div>
          <div style={{marginTop:'32px', display:'flex', flexDirection:'column', gap:'6px'}}>
            {nav.map(n=>{
              const active = path?.includes(n.href);
              return (
                <Link key={n.href} href={n.href} style={{padding:'12px 14px', borderRadius:'14px', fontWeight:700, fontSize:'14px', background: active ? '#BFFF00' : 'transparent', color: active ? 'black' : '#9A9AA0', border: '2px solid transparent', display:'flex', gap:'10px'}}>
                  {n.label}
                </Link>
              )
            })}
          ...
          <div style={{marginTop:'36px'}}>
            <div style={{fontSize:'10px', opacity:0.4, fontWeight:800, marginBottom:'12px'}}>APPLISTA</div>
            <div style={{display:'flex', flexDirection:'column', gap:'8px'}}>
              {apps.map(a=>(
                <div key={a.name} style={{display:'flex', justifyContent:'space-between', padding:'10px 12px', borderRadius:'12px', background: a.active ? '#1A1A1E' : 'transparent', border:'1px solid #1F1F23'}}>
        ...
        <div style={{borderTop:'1px solid #1A1A1E', paddingTop:'16px', display:'flex', gap:'10px', alignItems:'center'}}>
          ...
          <div><div style={{fontSize:'13px', fontWeight:800}}>Feffe</div><div style={{fontSize:'11px', opacity:0.5}}>Pro 1/50</div></div>
        </div>
      </aside>
      <main style={{flex:1, background:'#070708', minHeight:'100vh'}}><div style={{padding:'32px 40px', maxWidth:'1200px'}}>{children}</div></main>
    </div>
  )
}
